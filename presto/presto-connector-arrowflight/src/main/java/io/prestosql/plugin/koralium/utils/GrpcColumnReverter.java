...
import io.prestosql.plugin.koralium.KoraliumTypeOld;
...
public final class GrpcColumnReverter
{
    ...
    public static List<KoraliumExecutionColumn> BuildExecutionColumns(List<Presto.ColumnMetadata> columns)
    {
        ...
        for (int i = 0; i < columns.size(); i++) {
            ...
            KoraliumTypeOld type = KoraliumTypeOld.valueOf(column.getType().getNumber());
            KoraliumExecutionColumn executionColumn = new KoraliumExecutionColumn(type, column.getColumnId(), i, KoraliumTypeOld.CreatePrestoType(column));
